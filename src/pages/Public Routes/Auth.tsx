...
const useForm = (defaultValues = {}) => {
  ...
  const validateField = (name, value, rules) => {
    ...
    if (rules.required && (!value || value.trim() === '')) {
      ...
      error = rules.minLength.message || `${name} must be at least ${rules.minLength.value} characters`;
    }
  ...
  const handleSubmit = (onSubmit, validationSchema = {}) => (e) => {
    e.preventDefault();
        ...
        <div className="bg-[#1f2020] backdrop-blur-xl border border-white/10 rounded-3xl p-8 hover:shadow-amber-500/10 transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="space-y-6">
            ...
            {!isLogin && (
              <div className="grid grid-cols-2 gap-4 animate-fadeIn">
                <div className="space-y-2">
                  ...
                  />
                  {errors.firstName && (
                    <p className="text-red-400 text-xs mt-1 animate-fadeIn">{errors.firstName}</p>
                  )}
                </div>
                <div className="space-y-2">
                  ...
                  />
                  {errors.lastName && (
                    <p className="text-red-400 text-xs mt-1 animate-fadeIn">{errors.lastName}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <input
                {...register('email', isLogin ? loginValidationSchema.email : registerValidationSchema.email)}
                type="email"
                className={`w-full px-4 py-3 bg-gray-800/30 border rounded-xl text-white placeholder-gray-400 focus:outline-none transition-all duration-300 ${
                  errors.email 
                    ? 'border-red-500 focus:border-red-500 focus:ring-2 focus:ring-red-500/20' 
            ...
            <div className="space-y-2">
              ...
              <input
                {...register('password', isLogin ? loginValidationSchema.password : registerValidationSchema.password)}
                type="password"
                className={`w-full px-4 py-3 bg-gray-800/30 border rounded-xl text-white placeholder-gray-400 focus:outline-none transition-all duration-300 ${
                  errors.password 
                    ? 'border-red-500 focus:border-red-500 focus:ring-2 focus:ring-red-500/20' 
            ...
            {isLogin && (
              <div className="text-right">
                <button type="button" className="text-sm text-white hover:text-amber-300 transition-colors duration-200">
                  Forgot password?
                </button>
              </div>
            )}
            ...
            {/* Submit Button */}
            <button
              onClick={handleSubmit(onSubmit, isLogin ? loginValidationSchema : registerValidationSchema)}
              className="w-full py-3 bg-gradient-to-r from-yellow-500 to-amber-600 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500/50 transform hover:scale-[1.02] transition-all duration-200 shadow-lg hover:shadow-amber-500/25"
            >
          ...
          <div className="space-y-3">
            ...
            <button className="w-full py-3 bg-gray-800/50 border border-gray-600 text-white font-medium rounded-xl hover:bg-gray-700/50 transition-all duration-200 flex items-center justify-center space-x-2">
              <img src={Github} className="h-5 w-auto" style={{ filter: "brightness(0) invert(1)" }}/>
